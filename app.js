...
function process(data){
  ...
  data.forEach(r=>{
    ...
    const leader = mapped ? mapped.leader : ((r.added_by_leader || r.leader || "").toString().trim() || "Unmapped");
    // Use a task-group field from the ticket export when the agent is not in Str.
    // This keeps those agents with their team instead of creating a separate bucket.
    const group = mapped ? mapped.group : ((r.task_group || r.taskGroup || r.group || r.taskGroupName || "").toString().trim() || "Unmapped");
    ...
    if(!GLOBAL_DATA[group][leader][user]){
      GLOBAL_DATA[group][leader][user] = { hours:{}, total:0, reached:0, notReached:0, points:0 };
    }
...
function flattenGroup(leaders){
  const flat = {};
  for(const l in leaders) for(const a in leaders[l]) flat[a] = leaders[l][a];
  return flat;
}
...
function renderFiltered(){
  ...
  if(Object.keys(GLOBAL_DATA).length === 0){
    dashboard.innerHTML = `<div class="empty-state"><div class="big">No tickets loaded yet</div>Sync the mapping, then paste or upload the ticket export to build the board.</div>`;
    return;
  }

  let total=0, reached=0, nr=0, agentCount=0, maxCell=1;
  for(const g in GLOBAL_DATA) for(const l in GLOBAL_DATA[g]) for(const a in GLOBAL_DATA[g][l]){
  ...
  dashboard.innerHTML += `<div class="readout">
    <div class="cell"><div class="num">${total}</div><div class="label">Total tickets</div></div>
    <div class="cell"><div class="num">${reachPct(reached,nr)}%</div><div class="label">Reachability</div></div>
    <div class="cell"><div class="num">${(total/(GLOBAL_HOURS.length||1)).toFixed(1)}</div><div class="label">Productivity / hour</div></div>
    <div class="cell"><div class="num">${(total/agentCount || 0).toFixed(1)}</div><div class="label">Avg tickets / agent</div></div>
  </div>`;
  ...
  groupNames.forEach(g=>{
    ...
    summaryTotal.total += agg.total; summaryTotal.reached += agg.reached; summaryTotal.notReached += agg.notReached;
    summaryTotal.points += agg.points; summaryTotal.leaders += leaderCount; summaryTotal.agents += agentCountG;
    summary += `<tr><td class="name-cell"><span class="swatch" style="background:${groupColor(g)};color:${groupColor(g)}"></span>${g}</td><td>${agg.total}</td><td>${reachPct(agg.reached,agg.notReached)}%</td><td>${leaderCount}</td><td>${agentCountG}</td><td>${agg.points.toFixed(1)}</td></tr>`;
  });
  ...
  groupNames.forEach(g=>{
    ...
    groupEl.className = 'group'; groupEl.style.borderLeftColor = color; groupEl.open = !!search;
    ...
    leaderNames.forEach(l=>{ const p=aggregate(leaders[l]).points; if(p>topLeaderPts){topLeaderPts=p;topLeader=l;} });
    ...
    leaderNames.forEach(l=>{
      ...
      const leaderEl = document.createElement('details'); leaderEl.className='leader'; leaderEl.open=!!search;
      leaderEl.innerHTML = `<summary>${l === topLeader ? '<span class="crown">👑</span>' : ''}${l}<span class="lmeta"><span>${lAgg.total} tickets</span><span>${reachPct(lAgg.reached,lAgg.notReached)}% reach</span><span>${lAgg.points.toFixed(1)} pts</span><span>${Object.keys(agents).length} agents</span></span></summary>`;
      const wrap = document.createElement('div'); wrap.className='agents-wrap';
      const table = document.createElement('table'); table.className='agents';
      let head = `<thead><tr><th>Agent</th>`; GLOBAL_HOURS.forEach(h=>head+=`<th>${h}:00</th>`); head+=`<th>Total</th><th>Points</th><th>Reach %</th></tr></thead><tbody>`; table.innerHTML=head;

      const hourTotals = {};
      GLOBAL_HOURS.forEach(h=>hourTotals[h]=0);
      agentIds.sort((a,b)=>agents[b].total-agents[a].total).forEach(a=>{
        const u=agents[a], userReach=reachPct(u.reached,u.notReached), reachClass=(u.reached+u.notReached===0)?'':(userReach>=60?'reach-good':'reach-bad');
        let row=`<tr><td>${a}${COOR.includes(a)?' <span class="badge">COOR</span>':''}</td>`;
        GLOBAL_HOURS.forEach(h=>{const v=u.hours[h]||0; hourTotals[h]+=v; const alpha=v===0?0:0.14+0.62*(v/maxCell); row+=`<td class="hour-cell${alpha>0.5?' hot':''}" style="background:rgba(45,214,196,${alpha})">${v||''}</td>`;});
        row+=`<td class="total">${u.total}</td><td class="points">${u.points.toFixed(1)}</td><td class="${reachClass}">${userReach}%</td></tr>`;
        table.innerHTML += row;
      });

      let totalRow=`<tfoot><tr class="total-row"><th>Total</th>`;
      GLOBAL_HOURS.forEach(h=>totalRow+=`<th>${hourTotals[h]||''}</th>`);
      totalRow+=`<th>${lAgg.total}</th><th>${lAgg.points.toFixed(1)}</th><th>${reachPct(lAgg.reached,lAgg.notReached)}%</th></tr></tfoot>`;
...
function copyAsImage(){
  html2canvas(document.querySelector('.shell'),{backgroundColor:"#0a0e1a"}).then(canvas=>canvas.toBlob(blob=>{navigator.clipboard.write([new ClipboardItem({'image/png':blob})]);alert("Copied ✅");}));
}

document.addEventListener('DOMContentLoaded',()=>{
  document.getElementById("fileInput").addEventListener("change",e=>Papa.parse(e.target.files[0],{header:true,skipEmptyLines:true,complete:res=>process(res.data)}));
